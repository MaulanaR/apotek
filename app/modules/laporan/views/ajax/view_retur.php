...
<script>
    Highcharts.chart('chartview', {
        ...
        xAxis: {
            accessibility: {
                rangeDescription: 'Range tanggal : <?php echo date('d-m-Y', strtotime($tgl_awal)); ?> s/d <?php echo date('d-m-Y', strtotime($tgl_akhir)); ?>'
            },
    ...
    function export_x(par) {
        // Opens a new window
        myWindow = window.open("<?php echo base_url('laporan/export_retur/');?>?jenis="+par+"awal="+$("#tgl_awal_trans").val()+"&akhir="+$("#tgl_akhir_trans").val(), "Export", "width=800, height=600");
        // myWindow.close();
    }
</script>